...
if(file_exists($argv[1]))
{
	...
	{	
		...
		// پاک‌سازی: حذف هر چیزی جز رقم (اعشار ناخواسته اکسل مثل 09123456789.0 و غیره)
		...
		$phone = $number;
		// فرمت سلول Number در اکسل صفر ابتدایی رو حذف می‌کنه (مثلاً 09123456789 میشه 9123456789)
		// پس بر اساس طول شماره، صفر و پیش‌شماره‌ی خروجی رو خودمون برمی‌گردونیم:
		// ۱۰ رقم (بدون صفر، مثل 9123456789)  -> prefix + 0 + شماره
		// ۱۱ رقم (با صفر، مثل 09123456789)     -> prefix + شماره
		// ۱۲ رقم یا بیشتر (احتمالاً از قبل کامله) -> بدون تغییر
		$len = strlen($phone);
		if($prefix !== '' && $len <= 11) {
