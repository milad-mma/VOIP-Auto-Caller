...
| Upload Numbers | Import a number list (with per-row voice file) from Excel. ⚠️ Format the phone column as **Number, no decimals**. Enter the number **without** the leading 0 (e.g. `9123456789`) — Excel cannot keep a leading zero in Number format anyway. The app automatically restores the 0 and adds the configured outbound Prefix. Also, the `audio` column value must match an uploaded voice file's name **exactly** (no typos, no extension). |
