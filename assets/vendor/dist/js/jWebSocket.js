	...
	forceClose: function( aOptions ) {
		...
		if( this.isOpened() ) {
			this.sendToken({
				ns: jws.SystemClientPlugIn.NS,
				type: "login",
				username: user_extension,
				admin:admin_value,
				password: user_extension,
				encoding: lEncoding,
				pool: lPool,
				ip:asteriskip
			});
...
jws.oop.declareClass( "jws", "jWebSocketJSONClient", jws.jWebSocketTokenClient, {
	...
	tokenToStream: function( aToken ) {
		...
		//alert("lTOken to stream\n"+lJSON.toString(16));
		if(lJSON.indexOf("asynchronous_key") !== -1)
		{

			var mySplitResult = lJSON.split('jsonObject:');
			var mySplitResult1 = mySplitResult[1].split("{");
			var nm= mySplitResult1[1].split("}");
			var list = nm[0].split(",");
			//alert("length of list "+list.length);
			if(list.length==5)
			{
			
				UniqueID = window.UniqueID ;
				
				var lJSONs='{"type": "HangUp","ns": "'+jws.SystemClientPlugIn.NS+'","extension": "'+user_extension+'","UniqueID": "'+UniqueID+'","ip":"'+window.AsteriskIP+'"}';
				//alert(lJSONs);
				return lJSONs;
			}
			if(list.length==11)
			{
				//var lJSONs='{"ns": "org.jwebsocket.plugins.system","type": "header","clientType": "browser","clientName": "Chrome","clientVersion": "23.0.1271.64","clientInfo": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.11 (KHTML, like Gecko) Chrome/23.0.1271.64 Safari/537.11","jwsType": "javascript","jwsVersion": "1.0b8 (nightly build 20507)","jwsInfo": "native","utid": 1}';
				var lJSONs='{"ns": "'+jws.SystemClientPlugIn.NS+'","type": "header","clientType": "browser","clientName": "'+jws.getBrowserName()+'","clientVersion": "'+jws.getBrowserVersionString()+'","clientInfo": "'+navigator.userAgent+'","jwsType": "javascript","jwsVersion": "'+jws.VERSION+'","jwsInfo": "'+(jws.browserSupportsNativeWebSockets ? "native": "flash " + jws.flashBridgeVer)+'","utid": 1'+',"ip":"'+AsteriskIP+'"}';
				//alert("10--    "+lJSONs);
				return lJSONs;
			}
			if(list.length==8)
			{
				...
				var lJSONs='{"ns": "'+jws.SystemClientPlugIn.NS+'","type": "login","username": "'+user_extension+'","password": "'+user_extension+'","encoding": null,"pool": null,"utid": 2'+',"ip":"'+AsteriskIP+'"}';
				//alert("8--    "+lJSONs);
				return lJSONs;
			}
			if(list.length==7)
			{
				callto = window.dialNumber ;
				var lJSONs='{"type": "call","ns": "'+jws.SystemClientPlugIn.NS+'","cphonenumber": "'+callto+'","CustomContext": "'+window.DialPlan+'","CustomChannel": "'+window.DialoutPrefix+'","extension": "'+user_extension+'","ip":"'+AsteriskIP+'"}';
				//alert("8--    "+lJSONs);
				return lJSONs;
			}
			if(list.length==9)
			{
			transferTo = window.transferto;
				dialPlan = window.dialplan;
				UniqueID=window.UniqueID;
				var lJSONs='{"type": "TransferCall","UniqueID": "'+UniqueID+'","transfer": "null","identifier": "identifier","ns": "'+jws.SystemClientPlugIn.NS+'","TransferTo": "'+transferTo+'","CustomContext": "'+dialPlan+'","extension": "'+user_extension+'","ip":"'+AsteriskIP+'"}';
				//alert("8--    "+lJSONs);
				return lJSONs;
			}
